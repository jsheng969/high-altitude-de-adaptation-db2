  ...
  <div class="model-page">
    <div class="page-header">
      <div>
        ...
        <div class="page-desc">上传 Excel 文件并查看训练记录</div>
      </div>
    </div>
    ...
    <el-card shadow="never" class="card-wrap">
      ...
      <el-table v-loading="tableLoading" :data="list" border>
        ...
        <template #empty>
          <div class="table-empty">暂无训练记录</div>
        </template>
      </el-table>
    ...
    >
      <div v-loading="detailLoading">
        ...
        <template v-else-if="detailData">
          <!-- 图表：这里对应后端 plain_path，实际存的是谱系图 -->
          <el-card shadow="never" class="mb-16px" v-if="detailData.chart_path">
            <template #header>
              <div class="dialog-card-title">{{ chartTitle }}</div>
            </template>

            <div class="image-wrap" v-if="detailData.chart_preview_url">
              <img :src="detailData.chart_preview_url" :alt="chartTitle" />
            </div>
          ...
          <el-card shadow="never">
            <template #header>
              <div class="dialog-card-title">文件列表</div>
            </template>

            <div class="file-list">
              <div v-if="detailData.result_path" class="file-row">
                <div class="file-meta">
                  <div class="file-name-title">预测结果文件</div>
                  <!-- <div class="file-name">{{ detailData.result_path }}</div> -->
                </div>
                <el-button type="primary" plain @click="openDownload(detailData.result_download_url)">
                  下载
                </el-button>
              </div>

              <div v-if="detailData.cluster_path" class="file-row">
                <div class="file-meta">
                  <div class="file-name-title">聚类分析文件</div>
                  <!-- <div class="file-name">{{ detailData.cluster_path }}</div> -->
                </div>
                <el-button type="primary" plain @click="openDownload(detailData.cluster_download_url)">
                  下载
                </el-button>
              </div>

              <div v-if="detailData.plateau_path" class="file-row">
                <div class="file-meta">
                  <div class="file-name-title">高原数据文件</div>
                  <!-- <div class="file-name">{{ detailData.plateau_path }}</div> -->
                </div>
                <el-button type="primary" plain @click="openDownload(detailData.plateau_download_url)">
                  下载
                </el-button>
              </div>
            </div>
...
const chartTitle = computed(() => {
  if (props.datatype === '脑力') return '高原脑力数据谱系图'
  if (props.datatype === '体力') return '高原体力数据谱系图'
  return '高原精确操作数据谱系图'
})
  ...
  xhr.addEventListener('load', async () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
        ...
        successText.value = `文件「${file.name}」上传成功！${response.msg || '模型训练完成'}`
      } catch {
        showUploadStatus('success')
...
const loadRecords = async () => {
  ...
  try {
    ...
    total.value = 0
    totalPages.value = 1
    ElMessage.error(error?.message || '加载训练记录失败')
  } finally {
