package cn.iocoder.yudao.module.review.controller.admin.experimentsurveyhighlanddiseasesduringservice;
...
@Tag(name = "管理后台 - 实验组流调-高原期间出现的疾病")
@RestController
@RequestMapping("/review/experiment-survey-highland-diseases-during-service")
@Validated
public class ExperimentSurveyHighlandDiseasesDuringServiceController {
    ...
    @PostMapping("/create")
    @Operation(summary = "创建实验组流调-高原期间出现的疾病")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-highland-diseases-during-service:create')")
    ...
    @PutMapping("/update")
    @Operation(summary = "更新实验组流调-高原期间出现的疾病")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-highland-diseases-during-service:update')")
    ...
    @Operation(summary = "删除实验组流调-高原期间出现的疾病")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-highland-diseases-during-service:delete')")
    ...
    @GetMapping("/get")
    @Operation(summary = "获得实验组流调-高原期间出现的疾病")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-highland-diseases-during-service:query')")
    public CommonResult<ExperimentSurveyHighlandDiseasesDuringServiceRespVO> getExperimentSurveyHighlandDiseasesDuringService(@RequestParam("id") Long id) {
        ExperimentSurveyHighlandDiseasesDuringServiceDO experimentSurveyHighlandDiseasesDuringService = experimentSurveyHighlandDiseasesDuringServiceService.getExperimentSurveyHighlandDiseasesDuringService(id);
    ...
    @GetMapping("/page")
    @Operation(summary = "获得实验组流调-高原期间出现的疾病分页")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-highland-diseases-during-service:query')")
    public CommonResult<PageResult<ExperimentSurveyHighlandDiseasesDuringServiceRespVO>> getExperimentSurveyHighlandDiseasesDuringServicePage(@Valid ExperimentSurveyHighlandDiseasesDuringServicePageReqVO pageReqVO) {
        PageResult<ExperimentSurveyHighlandDiseasesDuringServiceDO> pageResult = experimentSurveyHighlandDiseasesDuringServiceService.getExperimentSurveyHighlandDiseasesDuringServicePage(pageReqVO);
        return success(BeanUtils.toBean(pageResult, ExperimentSurveyHighlandDiseasesDuringServiceRespVO.class));
    }

    @GetMapping("/export-excel")
    @Operation(summary = "导出实验组流调-高原期间出现的疾病 Excel")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-highland-diseases-during-service:export')")
    ...
    public void exportExperimentSurveyHighlandDiseasesDuringServiceExcel(@Valid ExperimentSurveyHighlandDiseasesDuringServicePageReqVO pageReqVO,
        ...
        List<ExperimentSurveyHighlandDiseasesDuringServiceDO> list = experimentSurveyHighlandDiseasesDuringServiceService.getExperimentSurveyHighlandDiseasesDuringServicePage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "实验组流调-高原期间出现的疾病.xls", "数据", ExperimentSurveyHighlandDiseasesDuringServiceRespVO.class,
