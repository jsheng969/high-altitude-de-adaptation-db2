package cn.iocoder.yudao.module.review.controller.admin.experimentsurveyselfassessment;
...
@Tag(name = "管理后台 - 实验组流调-自我评价")
@RestController
@RequestMapping("/review/experiment-survey-self-assessment")
@Validated
public class ExperimentSurveySelfAssessmentController {
    ...
    @PostMapping("/create")
    @Operation(summary = "创建实验组流调-自我评价")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-self-assessment:create')")
    ...
    @PutMapping("/update")
    @Operation(summary = "更新实验组流调-自我评价")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-self-assessment:update')")
    ...
    @Operation(summary = "删除实验组流调-自我评价")
    @Parameter(name = "id", description = "编号", required = true)
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-self-assessment:delete')")
    ...
    @GetMapping("/get")
    @Operation(summary = "获得实验组流调-自我评价")
    @Parameter(name = "id", description = "编号", required = true, example = "1024")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-self-assessment:query')")
    public CommonResult<ExperimentSurveySelfAssessmentRespVO> getExperimentSurveySelfAssessment(@RequestParam("id") Long id) {
        ExperimentSurveySelfAssessmentDO experimentSurveySelfAssessment = experimentSurveySelfAssessmentService.getExperimentSurveySelfAssessment(id);
        return success(BeanUtils.toBean(experimentSurveySelfAssessment, ExperimentSurveySelfAssessmentRespVO.class));
    }

    @GetMapping("/page")
    @Operation(summary = "获得实验组流调-自我评价分页")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-self-assessment:query')")
    public CommonResult<PageResult<ExperimentSurveySelfAssessmentRespVO>> getExperimentSurveySelfAssessmentPage(@Valid ExperimentSurveySelfAssessmentPageReqVO pageReqVO) {
        PageResult<ExperimentSurveySelfAssessmentDO> pageResult = experimentSurveySelfAssessmentService.getExperimentSurveySelfAssessmentPage(pageReqVO);
        return success(BeanUtils.toBean(pageResult, ExperimentSurveySelfAssessmentRespVO.class));
    }

    @GetMapping("/export-excel")
    @Operation(summary = "导出实验组流调-自我评价 Excel")
    @PreAuthorize("@ss.hasPermission('review:experiment-survey-self-assessment:export')")
    ...
    public void exportExperimentSurveySelfAssessmentExcel(@Valid ExperimentSurveySelfAssessmentPageReqVO pageReqVO,
        ...
        List<ExperimentSurveySelfAssessmentDO> list = experimentSurveySelfAssessmentService.getExperimentSurveySelfAssessmentPage(pageReqVO).getList();
        // 导出 Excel
        ExcelUtils.write(response, "实验组流调-自我评价.xls", "数据", ExperimentSurveySelfAssessmentRespVO.class,
